      ...
      <section className="relative pt-40 pb-20 border-b border-[#333333]">
        <div className="container mx-auto px-4 md:px-8">
          <div className="max-w-3xl">
            ...
            <h1 className="text-5xl md:text-7xl font-black tracking-tighter mb-6 uppercase">{t('howItWorks.title')} <br /><span className="text-[#C5A059]">{t('howItWorks.subtitle')}</span></h1>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-white/10 border border-white/10">
          {steps.map((step, i) => (
            <div key={i} className="p-10 bg-[#080B12] flex flex-col items-start hover:bg-white/5 transition-colors relative group">
              <span className="absolute top-6 right-6 text-3xl font-black text-white/5 group-hover:text-[#C5A059]/10 transition-colors">0{i+1}</span>
              ...
              <p className="text-slate-500 text-[9px] leading-relaxed">{step.desc}</p>
            </div>
          ))}
        </section>

        <section className="mt-20 p-12 border border-white/10 bg-[#080B12] text-center">
          <h2 className="text-2xl font-black uppercase tracking-tighter mb-8">{t('howItWorks.cta')}</h2>
          <Link to="/register">
            <Button className="bg-[#C5A059] hover:bg-[#B08D48] text-white rounded-none px-12 h-14 text-[10px] font-bold uppercase tracking-widest transition-all border-none">
              {t('howItWorks.ctaBtn')}
            </Button>
          </Link>
        </section>
